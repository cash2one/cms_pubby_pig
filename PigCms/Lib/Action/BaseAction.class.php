...
class BaseAction extends Action
{
    ...
    protected function all_insert($name='',$back='/index')
    {
        ...
        if($db->create()===false)
        {
            ...
            $id=$db->add();
            if($id){
                $m_arr=array('Img','Text','Voiceresponse','Ordering','Lottery','Host',
                    'Product','Selfform','Panorama','Wedding','Vote','Estate',
                    'Reservation','Greeting_card');
    ...
    protected function all_save($name='',$back='/index')
    {
        ...
        if($db->create()===false){
            ...
            $id=$db->save();
            if($id){
                $m_arr=array( 'Img', 'Text', 'Voiceresponse', 'Ordering','Lottery',
                    'Host','Product', 'Selfform', 'Panorama', 'Wedding', 'Vote',
                    'Estate', 'Reservation', 'Carowner','Carset'
                );
                if(in_array($name,$m_arr)){
                    $data['pid']=$_POST['id'];
                    $data['module']=$name;
    ...
    {
        $name=$name?$name:MODULE_NAME; $db=D($name);
        if($db->delete($id))
        {
