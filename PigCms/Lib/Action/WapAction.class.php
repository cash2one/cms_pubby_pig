...
class WapAction extends BaseAction
{
    ...
    protected function _initialize()
    {
        ...
        if (!$homeInfo||1){ $homeInfo=M('home')->where(array('token'=>$this->token))->find();
        ...
        if($radiogroup==false){ $radiogroup=0;
        }
	...
	$.post('".U('Share/shareData',array('token'=>$this->token,'wecha_id'=>$this->wecha_id))."',submitData,function (data) {},'json')
        ...
        </script>"; $this->assign('shareScript',$this->shareScript);
    }
    public function getLink($url)
    {
        ...
        }elseif ($this->strExists($url,'商家订单'))
        {
            ...
            else { $link='/index.php?g=Wap&m=Host&a=Detail&token='.$this->token.'&wecha_id='.$this->wecha_id;
        ...
        }elseif ($this->strExists($url,'全景'))
        {
            ...
            if ($itemid){ $link='/index.php?g=Wap&m=Panorama&a=item&token='.$this->token.'&wecha_id='.$this->wecha_id.'&id='.$itemid;
